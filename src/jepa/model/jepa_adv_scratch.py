...
class JepaAdv(nn.Module):
    def __init__(
        ...
        self.tgt_encoder_w1 = nn.Linear(in_dim, hidden_dim)
        self.tgt_encoder_w2 = nn.Linear(hidden_dim, in_dim)
        self.delta_wr1 = deepcopy(self.tgt_encoder_w1)
        self.delta_wr2 = deepcopy(self.tgt_encoder_w2)
        self.nd_act = nd_act()
    ...
    def forward(self, batch: dict) -> dict:
        ...
        perturbation = F.relu(self.tgt_encoder_w1(x_hat) + self.delta_wr1(x_hat)) 
        target_out = self.nd_act(self.tgt_encoder_w2(perturbation) + self.delta_wr2(perturbation)) # Formulation with negated gradient identity map
    ...
    @torch.no_grad()
    def encode(self, x):
        latent_code = self.tgt_encoder_w2(F.relu(self.tgt_encoder_w1(x)))
    ...
    def context_encode(self, x):
        latent_code = self.ctx_encoder_w2(F.relu(self.ctx_encoder_w1(x)))
        return latent_code # We always use this for the classifier, this way the forward function is only used for training.
